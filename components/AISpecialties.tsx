                ...
                <div className="flex flex-col lg:flex-row gap-16 items-center mb-20">
                    ...
                    <div className="lg:w-1/2 grid grid-cols-1 gap-6">
                        {specialties.map((item, idx) => (
                            <div key={idx} className="card-glass p-8 rounded-[2rem] border-[#AA771C]/10 hover:border-gold/30 transition-all duration-500 group flex items-center space-x-8">
                                <div className="w-16 h-16 rounded-2xl bg-[#F8F2DF] flex items-center justify-center text-gold border border-[#AA771C]/20 group-hover:bg-gold/10 group-hover:scale-110 transition-all duration-500 shrink-0">
                                    {item.icon}
                                </div>
                                <div>
                                    <div className="flex items-center space-x-3 mb-2">
                                        <h5 className="text-xl font-black text-[#101010] group-hover:text-gold transition-colors">{item.title}</h5>
                                        <span className="text-[9px] font-black uppercase text-gold/40 border border-gold/20 px-2 py-0.5 rounded">{item.tag}</span>
                                    </div>
                                    <p className="text-[#101010]/70 text-sm leading-relaxed">{item.description}</p>
                                </div>
                            </div>
                        ))}
                    </div>
