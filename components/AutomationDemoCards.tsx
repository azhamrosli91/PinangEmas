            ...
            <div className="grid md:grid-cols-3 gap-8 max-w-[1200px] mx-auto">
                {demos.map((demo, idx) => (
                    <div key={idx} className="group relative">
                        ...
                        <div className="card-glass p-8 rounded-[2.5rem] border-[#AA771C]/10 relative bg-white h-full flex flex-col hover:border-gold/20 transition-all duration-500">
                            <div className="flex items-center justify-between mb-8">
                                <div className="p-4 rounded-2xl bg-[#F8F2DF] text-gold group-hover:scale-110 transition-transform duration-500">
                                    {demo.icon}
                                </div>
                            ...
                            <h4 className="text-2xl font-black text-[#101010] mb-3">{demo.title}</h4>
                            ...
                            <div className="space-y-6 flex-grow">
                                {demo.steps.map((step, sIdx) => (
                                    <div key={sIdx} className="flex items-center space-x-4">
                                        <div className="w-10 h-10 rounded-xl bg-[#F8F2DF] flex items-center justify-center shrink-0 border border-[#AA771C]/20 group-hover:border-gold/30 transition-colors">
                                            {step.icon}
                                        </div>
                                        <span className="text-sm text-[#101010]/70 group-hover:text-[#AA771C]/80 transition-colors">{step.text}</span>
                                    </div>
                                ))}
                            </div>
