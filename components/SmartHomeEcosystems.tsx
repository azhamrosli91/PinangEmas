                ...
                <div className="flex flex-col lg:flex-row gap-20 items-center">
                    ...
                    <div className="lg:w-1/2 grid grid-cols-1 gap-6">
                        {ecosystems.map((eco, idx) => (
                            <div key={idx} className="card-glass p-10 rounded-[2.5rem] border-[#AA771C]/10 hover:border-gold/30 transition-all duration-500 group flex items-center space-x-8">
                                <div className="w-16 h-16 rounded-2xl bg-[#F8F2DF] flex items-center justify-center border border-[#AA771C]/20 group-hover:scale-110 group-hover:bg-[#AA771C]/10 transition-all duration-500 shrink-0">
                                    {eco.icon}
                                </div>
                                <div>
                                    <h5 className="text-2xl font-black text-[#101010] mb-2 group-hover:text-gold transition-colors">{eco.name}</h5>
                                    <p className="text-[#101010]/55 text-base leading-relaxed">{eco.description}</p>
                                </div>
                            </div>
                        ))}
                    </div>
